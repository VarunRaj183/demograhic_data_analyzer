...
def calculate_demographic_data(print_data=True):
    ...
    avg_age = df.query('sex == "Male"').age.mean()
    average_age_men = round(avg_age,1)

    # What is the percentage of people who have a Bachelor's degree?
    ed = df.query('education == "Bachelors"').education.count()
    l = len(df)
    per = (ed/l)*100
    percentage_bachelors = round(per,1) 
    ...
    io = df[df["native-country"].isin(["India"])]
    top_IN_occupation = io[io["salary"] == ">50K"].value_counts("occupation").index[0]
